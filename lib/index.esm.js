 ...
 */
const Button = (_a) => {
    var { variant = "primary", bgColor, label, labelColor, isLoading = false } = _a, props = __rest(_a, ["variant", "bgColor", "label", "labelColor", "isLoading"]);
    if (isLoading) {
        props.disabled = true;
    }
    return (React.createElement("button", Object.assign({ type: "button", className: `button button--${variant}`, style: { color: labelColor, backgroundColor: bgColor } }, props), isLoading ?
        (React.createElement("div", { style: { display: "flex", alignItems: "center", fontSize: "1em", lineHeight: "normal" } },
            React.createElement("div", { className: "loader", style: props.loadingLabel ? { marginRight: "0.5rem" } : { fontSize: "1rem" } }),
            props.loadingLabel)) : (label)));
};
